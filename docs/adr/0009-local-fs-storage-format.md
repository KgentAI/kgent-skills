# 0009 — local-fs 存储格式：markdown + frontmatter，wiki 形目录树，路径即 id
...
frontmatter 的 `version` 行兼任 git revert 的冲突哨兵：任何后续 skill 写都会改动该行，使对旧写 commit 的 revert 必然冲突——这是 0010 freshness 的结构性一层。

删除与 undo 属于 store 的版本化机制，按有效模式两档：**ADR 0010**（git-backed：store 是 git 仓库，undo 是 revert）与 **ADR 0011**（snapshot：git 不可用时的快照兜底，`.trash` 回归）。
...
- 手工编辑不 bump `version`，写路径 CAS 会放行（skill 层无法察觉同刻外部修改）；hash 校验与 git 的 dirty-tree 检查（0010）在 undo 处兜底——已知限制，记入 skill。
