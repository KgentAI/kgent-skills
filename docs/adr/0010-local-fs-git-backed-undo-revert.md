# 0010 — local-fs store 是 git 仓库，undo 是 revert（git-backed 模式）

> **修订**：git 不再是硬依赖——git 不可用（或 root 嵌于他人仓库）时 local-fs 降档为快照兜底模式，见 **ADR 0011**。本 ADR 描述 git-backed 模式。
...
- git-backed 模式下 git 是硬依赖：缺失时 local-fs 降档 snapshot 模式（0011），不拒启；setup/doctor 检查并报告有效模式。
