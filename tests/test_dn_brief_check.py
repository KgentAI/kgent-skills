...
def test_cited_kgent_uri_exit1(tmp_path):
    """引用来源 带 kgent:// → 违规（Q20/S74：只准原生 URL）。"""
    nodes = "- Q1 [引用来源|kgent://lark/abc] 现供应商报价"
    mermaid = "```mermaid\nflowchart TD\n    Q1\n```"
    result = run_checker(tmp_path, _brief(mermaid=mermaid, nodes=nodes))
    assert result.returncode == 1
...
def test_noncited_state_with_url_suffix_exit1(tmp_path):
    ...
    nodes = "- Q1 [假设|https://example.com/x] 预算上限"
    mermaid = "```mermaid\nflowchart TD\n    Q1\n```"
    result = run_checker(tmp_path, _brief(mermaid=mermaid, nodes=nodes))
    assert result.returncode == 1
...
def test_empty_node_list_exit1(tmp_path):
    """零问题（图在表不在）→ 违规。"""
    result = run_checker(tmp_path, _brief(nodes="（本轮无问题）"))
    assert result.returncode == 1
...
def test_divergent_edge_mermaid_only_exit1(tmp_path):
    """mermaid 有边、问题表缺 依 → 违规（边集不等，mermaid 侧多出）。"""
    mermaid = "```mermaid\nflowchart TD\n    Q1 --> Q2\n```"
    nodes = "- Q1 [假设] A\n- Q2 [假设] B"
    result = run_checker(tmp_path, _brief(mermaid=mermaid, nodes=nodes))
    assert result.returncode == 1
...
def test_divergent_edge_list_only_exit1(tmp_path):
    """问题表有 依、mermaid 缺边 → 违规（边集不等，表侧多出）。"""
    mermaid = "```mermaid\nflowchart TD\n    Q1\n    Q2\n```"
    nodes = "- Q1 [假设] A\n- Q2 [假设] B 依:Q1"
    result = run_checker(tmp_path, _brief(mermaid=mermaid, nodes=nodes))
    assert result.returncode == 1
...
def test_list_node_missing_from_mermaid_exit1(tmp_path):
    """问题表问题不在 mermaid → 违规（覆盖双向）。"""
    mermaid = "```mermaid\nflowchart TD\n    Q1\n```"
    nodes = "- Q1 [假设] A\n- Q5 [假设] E"
    result = run_checker(tmp_path, _brief(mermaid=mermaid, nodes=nodes))
    assert result.returncode == 1
    assert "Q5" in result.stdout
...
def test_converged_over_cap_exit1(tmp_path):
    """第5轮完成 超默认上限 3 且无达限/延长 → 违规。"""
    result = run_checker(tmp_path, _brief(convergence="第5轮完成：零新增。"))
    assert result.returncode == 1


def test_extension_recorded_exit0(tmp_path):
    """达限 + 用户批准延长 + 第5轮完成 → 合法（exit 0）。"""
    convergence = "第3轮达限，经用户批准延长3轮；第5轮完成：零新增。"
...
def test_cap_reached_assumption_path_exit0(tmp_path):
    """达限 + 余下按假设 → 合法（exit 0）。"""
    convergence = "3轮达限，未完成；余下开放问题按假设处理。"
    result = run_checker(tmp_path, _brief(convergence=convergence))
    assert result.returncode == 0, result.stdout + result.stderr


def test_custom_max_passes_flag(tmp_path):
    """--max-passes 提高上限后 第5轮完成 合法。"""
    result = run_checker(tmp_path, _brief(convergence="第5轮完成：零新增。"), "--max-passes", "5")
    assert result.returncode == 0, result.stdout + result.stderr


def test_converged_over_extended_cap_exit1(tmp_path):
    """延长后仍超总上限（延长3 → 总 6，第7轮完成）→ 违规。"""
    convergence = "第3轮达限，经用户批准延长3轮；第7轮完成。"
    result = run_checker(tmp_path, _brief(convergence=convergence))
    assert result.returncode == 1
...
def test_undecodable_file_exit2(tmp_path, encoding):
    ...
    brief = tmp_path / "brief.md"
    brief.write_text("第2轮完成", encoding=encoding)
    result = subprocess.run(
