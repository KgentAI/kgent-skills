...
- **分区单位 = eval 文件**（`--parallel` 自动按文件分组）。写域不相交的概率
  最高：只读组（query-knowledge，零台账写入）与创建组（wiki-setup，新建
  独立页）可安全并行。decision-navigator 属只读组（全程零写路径，spec
  2026-09-11 B12 卫兵），与 query-knowledge 同组并行；其逐轮评审门（ADR 0014）
  在本 runner（无 Task 工具）下天然走**内联复查**路径——transcript 应见
  `【评审门 · 执行方式：内联复查】` 标记；真派发路径的 eval 断言随 runner
  Task 扩展另行补（spec 2026-09-20 分期裁决）。
