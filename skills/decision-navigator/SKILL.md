...
能力盘点。流程裁决见 `docs/adr/0012`（检索级联）与 `docs/adr/0013`（收敛循环）。
...
- **根节点** = 用户在请求里提出的原始问题（整张图的顶层目标）
- 子节点 = 为解算父节点需要先解算的子问题/变量；边 = 解算依赖，**箭头从子
  指向父**（`N1 --> N2` 读作「N1 是 N2 的子问题：先知道 N1 才能解 N2」）
- 拆到「没有它就无法在候选间比较」为止——不拆与排序无关的问题
- **每次图有变更（新增节点、新解算、重开）时立即向用户展示当前图**——不只
  在简报里呈现终版图；让用户看到拆解进展与每个节点的当前状态
- 节点标签里带状态标记（见简报格式节的状态枚举）

### 3. 分解⇄检索收敛循环（≤3 轮；ADR 0013）
...
5. **变更报告 + 展示更新后的图**：本轮新增了什么节点、重开了什么、新解算了
   什么。**向用户展示当前完整的依赖图**（mermaid + 节点表），每个节点标签带
   状态标记——让用户看到拆解进展与每个节点的当前状态。**新增、重开、新增引
   用解算全为零 → 收敛**，出循环（假设级变化不计为进展，不构成再来一轮的理
   由）
...
- 首行必须恰为 `flowchart TD`；行只准两种：`N1["标签 · 状态"]` 与 `N1 --> N2`
- **节点标签带状态**（`标签 · 状态`）——让用户在图上直接看到每个节点的解算进度
- 状态枚举同节点表：`用户已给` / `引用来源` / `假设` / `依前未解`
- `引用来源` 状态不在标签里带 URL（URL 住在节点表）
- 虚线/粗线/带字边/subgraph/样式一律不用
- 图与节点表必须逐边一致：边集合相等、节点双向覆盖、无环（DAG）
- **根节点**（用户原始问题）在图顶部，子问题箭头向上指向父问题
