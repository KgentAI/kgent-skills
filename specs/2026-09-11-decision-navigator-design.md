...
- **Discovered while:** 维护者提出 Decision-Guidance Agent Skill Architecture 设计稿
  ...
  `CONTEXT.md`，检索层裁决落 `docs/adr/0012`，流程裁决落 `docs/adr/0013`
...
> 新 ADR 撞号，改编号为 **0012/0013**（正文引用已同步）。⚠️ 上游遗留：main 的
...
| 原设计稿 | 本 spec | 依据 |
|---|---|---|
| 向量库 + 知识图谱 + 内部数据库连接器 | 检索级联：kgent 联邦知识库 → 能力盘点发现的内部源 → 宿主 web（机会性）；向量库否决，知识图谱**延后**（经能力盘点接入） | ADR 0012 |
| 线性四阶段（检索→分解） | 分解⇄检索收敛循环（分解先行、定点收敛） | ADR 0013 |
...
| 「动态更新权重」 | 节点重开 + 下游传播；澄清批轮界合并 | ADR 0013 Consequences |
...
3. **分解⇄检索**：收敛循环——strawman 子问题图 → 按开放节点信息需求定向检索 →
   ...
   达限征询用户延长（N 用户定，默认 3）；假设级变化不计为进展（ADR 0013）
...
9. **零证据降级**：级联全空 → 不拒绝；纯假设推演，置信度钉
   「低——纯假设推演」，「先补数据」列为推荐下一步；前提本身未经证实
   （如「听说 X 部门在试点」查无实据）同走此路径，此时排序挂起——说破
   「本轮不排序」+ 只排下一步行动（B9 在此路径不适用，见 B10）
...
- 不自建向量库/嵌入/索引；不接企业知识图谱（延后接入点 = 能力盘点，ADR 0012）
...
- **B9 排序呈现**（结构 + 人工复核）：排序 + 逐方案权衡在场；与权重表一致
  性入 rubric-judge 待办（本期人工复核）。零证据降级路径例外：排序挂起
  （说破即可 + 只排下一步行动），不视为 B9 违规。
- **B10 零证据降级**（eval）：构造级联全空场景 → 简报含「低——纯假设推演」
  置信度 + 「先补数据」推荐；且不伪造加权排序——排序挂起须说破。
